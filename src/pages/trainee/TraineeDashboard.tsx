...
import { 
  BookOpen, CheckCircle, Award, CheckSquare, GraduationCap, 
...
export const TraineeDashboard: React.FC<TraineeDashboardProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 max-w-7xl mx-auto w-full">
      ...
      <div className="bg-gradient-to-r from-blue-900 via-blue-800 to-indigo-900 rounded-3xl p-5 sm:p-8 text-white shadow-xl relative overflow-hidden">
        <div className="relative z-10 max-w-2xl">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-500/20 border border-blue-400/30 text-blue-200 text-xs font-bold uppercase tracking-wider mb-3">
            ...
            <span>Trainee Learning Portal • CAPACITY CONNECT</span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-3 mt-6">
            {activeCourse && (
              <button
                ...
                className="min-h-[44px] px-5 py-2.5 bg-white text-blue-900 hover:bg-blue-50 font-bold text-xs rounded-xl shadow-xs transition-colors flex items-center justify-center gap-2 cursor-pointer"
              >
            ...
            <button
              ...
              className="min-h-[44px] px-4 py-2.5 bg-blue-700/60 hover:bg-blue-700 text-white font-semibold text-xs rounded-xl border border-blue-400/30 transition-colors flex items-center justify-center cursor-pointer"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {activeCourse && activeEnrollment && (
            <div className="bg-white dark:bg-[#151B28] rounded-3xl border border-slate-200 dark:border-slate-800 p-5 sm:p-6 shadow-2xs transition-colors">
              <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
                <div className="flex items-center gap-2">
                  <BookOpen className="w-5 h-5 text-blue-600 dark:text-blue-400 shrink-0" />
                  <h2 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white">Current In-Progress Course</h2>
                </div>
              ...
              <div className="flex flex-col sm:flex-row gap-5 items-start">
                <img
                  ...
                  className="w-full sm:w-44 h-32 sm:h-28 rounded-2xl object-cover shrink-0"
                />
                <div className="flex-1 min-w-0 w-full">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded border border-slate-200 dark:border-slate-700">
                      {activeCourse.category}
                    </span>
                    <span className="text-xs text-slate-500 dark:text-slate-400">• {activeCourse.trainerName}</span>
                  </div>
                  <h3 className="font-bold text-slate-900 dark:text-white text-sm sm:text-base mb-2">{activeCourse.title}</h3>
                  <p className="text-xs text-slate-600 dark:text-slate-400 line-clamp-2 mb-4">{activeCourse.description}</p>
                  ...
                  <button
                    onClick={() => onNavigate('course-learning', { courseId: activeCourse._id })}
                    className="w-full sm:w-auto min-h-[44px] sm:min-h-[40px] px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-xl shadow-xs transition-colors flex items-center justify-center gap-2 cursor-pointer"
                  >
          ...
          {pendingAssessments.length > 0 && (
            <div className="bg-amber-50/80 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/40 rounded-3xl p-5 sm:p-6 shadow-2xs">
              <div className="flex items-center gap-2.5 mb-4 text-amber-900 dark:text-amber-300">
              ...
              <div className="space-y-3">
                {pendingAssessments.map((assessment) => (
                  <div
                    key={assessment._id}
                    className="bg-white dark:bg-[#151B28] p-4 rounded-2xl border border-amber-200 dark:border-amber-900/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-2xs"
                  >
                    <div>
                      <h4 className="font-bold text-slate-900 dark:text-white text-xs sm:text-sm">{assessment.title}</h4>
                    ...
                    <button
                      ...
                      className="min-h-[44px] sm:min-h-[40px] px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold rounded-xl transition-colors shrink-0 shadow-2xs flex items-center justify-center cursor-pointer"
                    >
                      Start Assessment
                    </button>
                  </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                onClick={() => onNavigate('courses-explore')}
                className="text-xs font-bold text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 min-h-[44px] flex items-center cursor-pointer"
              >
                View all
              </button>
            </div>
          ...
          <div className="bg-white dark:bg-[#151B28] rounded-3xl border border-slate-200 dark:border-slate-800 p-5 sm:p-6 shadow-2xs transition-colors">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-slate-900 dark:text-white text-sm">Competency Matrix</h3>
              </div>
              <span className="text-xs font-extrabold text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-500/10 px-2 py-0.5 rounded-lg border border-indigo-200 dark:border-indigo-500/30">
              ...
              {analytics.competencyPillars.map((pillar, idx) => (
                <div key={idx}>
                  <div className="flex justify-between text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">
                    ...
                    <span className="font-bold text-slate-900 dark:text-white">{pillar.score}%</span>
                  </div>
            ...
            <button
              ...
              className="w-full min-h-[44px] mt-5 py-2.5 px-3.5 bg-slate-50 dark:bg-slate-800/80 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-bold rounded-xl border border-slate-200 dark:border-slate-700 transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
            >
          ...
          <div className="bg-white dark:bg-[#151B28] rounded-3xl border border-slate-200 dark:border-slate-800 p-5 sm:p-6 shadow-2xs transition-colors">
            ...
            <div className="space-y-3">
              {activityLogs.length === 0 ? (
                <p className="text-xs text-slate-400 dark:text-slate-500">No recent activity logged.</p>
              ) : (
                activityLogs.map((log) => (
                  <div key={log._id} className="text-xs flex items-start gap-2.5 pb-2.5 border-b border-slate-100 dark:border-slate-800 last:border-none">
                    <div className="w-2 h-2 rounded-full bg-blue-600 dark:bg-blue-400 shrink-0 mt-1.5" />
                    <div className="min-w-0">
                      <p className="font-semibold text-slate-800 dark:text-slate-200 truncate">{log.action}: {log.details}</p>
                      <p className="text-[10px] text-slate-400 dark:text-slate-500">{new Date(log.timestamp).toLocaleString()}</p>
                    </div>
                  </div>
                ))
              )}
